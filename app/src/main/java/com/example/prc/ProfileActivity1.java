...
public class ProfileActivity1 extends AppCompatActivity
{
    ...
    private final int GalleryPick = 100;
        ...
        if (ImageUri == null)
        {
            userRef.addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(DataSnapshot dataSnapshot)
                {
                    ...
                    {
                        Toast.makeText(ProfileActivity1.this, "Select image first...", Toast.LENGTH_SHORT).show();
                    }
                }
                    ...
                    if (task.isSuccessful())
                    {
                        ...
                        userRef.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).updateChildren(profileMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                            ...
                            public void onComplete(@NonNull Task<Void> task)
                            {
                                if (task.isSuccessful())
                                {
                                    Intent intent = new Intent(ProfileActivity1.this,MainActivity2.class);
                                    startActivity(intent);
                                    finish();
                                    progressDialog.dismiss();
                                    Toast.makeText(ProfileActivity1.this, "Profile settings has been updated", Toast.LENGTH_SHORT).show();
                                }
                            }
                        });
                    }
                ...
                .addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot)
                    {
                        if(dataSnapshot.exists())
                        {
                            ...
                            Picasso.get().load(imsgeDb).placeholder(R.drawable.profile_image).into(profileImageView);
                        }
                    }
